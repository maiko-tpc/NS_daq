...
/* functions for V560 scaler */
void v560_map_scale_clear(int nmap){
  short sval;
  sval=1;
  univ_map_write16(V560_SCA_CLE, &sval, nmap);  
}

int v560_map_ridf_nscaler(int nmap){
  int i;
  for(i=0; i<V560_NCH; i++){
    univ_map_read32(V560_COUNTER_0+i*4, (long *)(data+mp), nmap);
    mp+=2;
  }
  scrsize+=2*V560_NCH;
  if(eventsize>0) eventsize+=2*V560_NCH;
  return scrsize;
}
